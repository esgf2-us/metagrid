...
@csrf_exempt
def get_globus_state(request):
    if not request.method == "POST":
        # pragma: no cover
        return HttpResponseBadRequest("Request method must be POST.")

    if request.session.get("globus_state") is None:
        return HttpResponse({"result": "Globus state not found!"})

    response = {
        "result": "Globus state found!",
        "sessionState": request.session.get("globus_state"),
    }
...
def set_globus_state(request):
    ...
    if requestBody is not None:
        request.session["globus_state"] = requestBody

        request.session.modified = True
        response = {
            "result": "Globus state updated!",
            "sessionState": requestBody,
        }
        return HttpResponse(json.dumps(response))

    return HttpResponse(
        {"Error": "Request body was None. Globus state unchanged."}
    )
