...
export const saveDataToLocal = (key: string, data: Object): void => {
  const globusState = localStore.getItem('globusState');
  if (globusState) {
    const localState = JSON.parse(globusState);
    const newLocalState = { ...localState, [key]: JSON.stringify(data) };
    localStore.setItem('globusState', JSON.stringify(newLocalState));
  } else {
    localStore.setItem('globusState', JSON.stringify({ [key]: data }));
  }
};
...
export const removeFromLocal = (key: string): void => {
  ...
  if (globusState) {
    const localState = JSON.parse(globusState);
    const { [key]: unused, ...newLocalState } = localState;
    localStore.setItem('globusState', JSON.stringify(newLocalState));
  }
};
...
export const getDataFromLocal = <T>(key: string): T | null => {
  ...
  if (globusState) {
    const localState = JSON.parse(globusState);
    const dataString = localState[key];
